...
#test_count<-ple_counts[rownames(ple_counts) == 'PLE_TRINITY_DN4929_c0_g1_i2',]
#test<-DGEList(counts= test_count,group=factor(ple_groups))
#cpm(test)
#rowSums(cpm(test)>1)
#ple_d.full<-ple_d
...
# create DGE list
# keep only rows where counts per million is above 100 in at least 2 samples (if using, comment out otherwise)
